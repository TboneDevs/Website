#!/usr/bin/env python3
"""
CPM Web Tool - Fixed Version for Railway
"""
...
def run_async(coro):
    """Run async functions safely from Flask"""
    try:
...
def login():                    # ← Changed from async to normal def
    ...
    # Login using asyncio.run (safe in Flask)
    ...
    # Save token and load account data
...
def action(action):
    ...
    # All actions use the safe run_async helper
    if action == "set_money":
        ...
        feats = [
            nuker.unlock_w16(web_uid), nuker.unlock_horns(web_uid),
            nuker.disable_damage(web_uid), nuker.unlimited_fuel(web_uid),
            nuker.unlock_smoke(web_uid), nuker.unlock_animations(web_uid),
            nuker.unlock_wheels(web_uid), nuker.unlock_houses(web_uid),
